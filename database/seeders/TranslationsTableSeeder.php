...
class TranslationsTableSeeder extends Seeder
{
    ...
    private function categoriesTranslations()
    {
        ...
        //
        $cat = Category::where('name', 'category-1')->firstOrFail();
        if ($cat->exists) {
            $this->trans('pt', $this->arr(['categories', 'name'], $cat->id), 'categoria-1');
            $this->trans('pt', $this->arr(['categories', 'name'], $cat->id), 'Categoria 1');
        }
        $cat = Category::where('name', 'category-2')->firstOrFail();
        if ($cat->exists) {
    ...
    {
        $page = Page::where('name', 'hello-world')->firstOrFail();
        if ($page->exists) {
             ...
             * ```
             */
            $_arr = $this->arr(['pages', 'name'], $page->id);
