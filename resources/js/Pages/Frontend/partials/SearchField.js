...
const SearchField = () => {
    ...
    const {trans, classNames, headerColor, headerBgColor, isAdminOrAbove, textColor } = useContext(AppContext)
    ...
    return (
        <div
            className="hidden xl:flex flex-row mb-1">
            ...
            <div
                ...
                <Link
                    ...
                    className="px-5">
                    <SearchIcon className={`h-5 w-5 ${textColor}`}
                                aria-hidden="true"/>
                </Link>
                {
                    requestType === 'backend' ? <Link
                        href={route().has(`${requestType}.${searchType}.index`) ? route(`${requestType}.${searchType}.index`) : '#'}
                        className="px-5">
