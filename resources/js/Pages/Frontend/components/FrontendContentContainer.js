...
export default function({children, showBreadCrumbs = true, childName = '', parentModuleName = null}) {
    const { mainBgColor , getTheme } = useContext(AppContext);
    return (
        <div className={`bg-${mainBgColor}-${getTheme(50,700)} w-full md:w-full lg:w-full xl:w-4/5 2xl:w-3/5 m-auto shadow-xl min-h-screen pb-10 text-sm`}>
