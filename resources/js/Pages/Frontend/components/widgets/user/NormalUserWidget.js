...
const NormalUserWidget =  ({ element }) => {
    ...
    return (
        ...
        <Link href={route('frontend.user.show', element.id) + `?slug=${element[getLocalized()].replace(/ /g, '-')}`}
            ...
            <div className="w-auto h-auto flex items-center justify-center aspect-w-1 aspect-h-1 mb-4">
                <img
                    src={getThumb(element.image)}
                    alt={element[getLocalized()]}
                    className="z-0 w-4/4 h-auto rounded-full shadow-md object-center object-cover group-hover:opacity-75"
                    width={360}
                    height={480}
                    loading='lazy'
                />
            </div>
            <h3 className={`text-center text-${mainColor}-800 dark:text-${mainColor}-100 truncate text-sm`}>
