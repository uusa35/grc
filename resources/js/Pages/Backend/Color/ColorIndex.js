                            ...
                            <table className="min-w-full border-collapse block md:table">
                                        ...
                                        <div className="flex flex-row justify-between items-center">
                                            ...
                                            <div className="flex">
                                                <Link
                                                    ...
                                                    href={route('backend.color.create', {product_id: elements.data.length > 0 ? elements.data[0]?.product_id : params.product_id})}>
                                ...
                                {map(elements.data, a => (
                                    <tr key={a.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm ">{a[getLocalized()]}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                                            <div className="flex flex-row justify-start content-center ">
                                                <div>{a.code}</div>
                                                <div className="w-4 h-4 mx-5 border-gray-100 border-2 rounded-full" style={{ backgroundColor : a.code}}></div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm ">
                                            <div className="flex flex-row items-center justify-around">
                                                <Link href={route(`backend.color.edit`, a.id)}
                                                ...
                                                <button
                                                    onClick={() =>
                                                        dispatch(showModal({
                                                            ...
                                                            model: 'color',
                                                            id: a.id,
                                                            title: `${trans('destroy')} ${trans('color')} ${a[getLocalized()]}`,
                                                            message: `${trans('confirmation')} ${trans('destroy')} ${trans('color')}`,
                                                        }))
                                                    }
                                                    className="text-gray-600 hover:text-gray-900 ">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6"
