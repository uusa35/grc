...
export default function(cart = initialState, action) {
    ...
    switch (action.type) {
        ...
        case REMOVE_FROM_CART: // only cart_id
            ...
            currentTotalItems = parseInt(sumBy(map(items, item => item.cart_id === action.payload.cart_id ? action.payload : item), 'qty'))
            currentTotalWeight = round(parseFloat(sumBy(cart.items, 'weight')), 2);
            return {
                ...cart,
        ...
        case SET_DISCOUNT : // only discount value
            ...
            currentShipmentFees = cart.shipmentFees;
            currentDiscount = parseFloat(action.payload.is_percentage ? currentTotal * (action.payload.value / 100) : action.payload.value);
